...
public class JdbcConnectionRepository implements ConnectionRepository {
	...
	public void addConnection(Connection<?> connection) {
		try {
			...
			int rank = jdbcTemplate.queryForObject("select coalesce(max(rank) + 1, 1) as rank from " + tablePrefix + "user_connection where user_id = ? and provider_id = ?", new Object[]{ userId, data.getProviderId() }, Integer.class);
			jdbcTemplate.update("insert into " + tablePrefix + "user_connection (user_id, provider_id, provider_user_id, rank, display_name, profile_url, image_url, access_token, secret, refresh_token, expire_time) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
	...
	private String selectFromUserConnection() {
		return "select user_id, provider_id, provider_user_id, display_name, profile_url, image_url, access_token, secret, refresh_token, expire_time from " + tablePrefix + "user_connection";
	}
